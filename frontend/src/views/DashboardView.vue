    ...
    <main class="relative pt-32 pb-16 px-6" style="z-index:2">
      <div class="max-w-6xl mx-auto">
        ...
        <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-6 mb-10">
          <div>
            <p class="text-orange-400 text-sm font-medium mb-1">
              <!-- The /dashboard route guard (router/index.js) means we only
                   ever land here once auth.user is set, so no guest fallback. -->
            ...
            <p class="text-sm mt-1" :class="theme.isDark ? 'text-gray-500' : 'text-gray-600'">
              Manage your uploads, generate captions, and dub with AI voices.
            </p>
          </div>
        ...
        <div v-else-if="store.videos.length === 0"
          ...
          <p class="mb-8" :class="theme.isDark ? 'text-gray-500' : 'text-gray-600'">Upload your first video to get started with AI dubbing.</p>
              ...
              <div class="p-4">
                ...
                <div class="flex items-center justify-between">
                  <span class="text-xs text-gray-500">
                    {{ v.status === 'dubbed' ? '✅ Ready to share' : v.status === 'transcribed' ? '📝 Edit, translate, or dub' : '⬆️ Uploaded' }}
                  </span>
...
// Mirrors backend/models/video.js status field: "uploaded" -> "transcribed" -> "dubbed".
// (Translating doesn't currently move video.status — only the Caption doc changes.)
const statusTabs = ["all", "uploaded", "transcribed", "dubbed"];
...
const quickStats = computed(() => [
  ...
  { label: "Transcribed",  value: store.videos.filter(v => ["transcribed", "dubbed"].includes(v.status)).length },
  { label: "Dubbed",       value: store.videos.filter(v => v.status === "dubbed").length },
  { label: "Uploaded",     value: store.videos.filter(v => v.status === "uploaded").length },
]);

const statusClass = (s) => ({
  uploaded:    "bg-gray-800/80 text-gray-300 border-gray-700",
  transcribed: "bg-blue-900/80 text-blue-300 border-blue-700",
  dubbed:      "bg-green-900/80 text-green-300 border-green-700",
...
const tips = [
  ...
  { icon: "🎭", title: "Dub with AI voices",       desc: "Click 'AI Dub' to generate a distinct AI voice per speaker and mux it onto your video." },
];
</script>